...
public class Automobile implements Serializable{
	...
	/**
	 * Used to get the price of an option
	 * @param opsetsname - name of the option set that contains the option
	 * @param optname - name of the option 
	 * @return - price of the option
	 */
	public float getOptionPrice(String opsetsname,String optname){
		Option opt = this.findOption(opsetsname, optname);
		return opt.getPrice();
		
	}
	
	/**
	 * Used to get the names of all the optionsets that are availabe in an automobile
	 * @return
	 */
	public ArrayList<String> getOptionsetNames(){
		ArrayList<String> arr = new ArrayList<String>();
		for(OptionSet os : this.getOpsets()){
			arr.add(os.getName());
		}
		return arr;
	}
	...
	/**
	 * Used to get the names of all the options that are present in an optionset
	 * @param opsetName
	 * @return
	 */
	public ArrayList<String> getOptionNames(String opsetName){
		ArrayList<String> arr = new ArrayList<String>();
		OptionSet os = this.findOpset(opsetName);
		for(Option opt : os.getOpts()){
			arr.add(opt.getName());
		}
		return arr;
	}
	/**
	 * Allows the user to select a model's respective options.
	 */
	public void selectOptions(){
		...
		for(OptionSet os : this.getOpsets()){
			validOption = false;
			System.out.println("Choose the option for the optionset :"+os.getName());
			System.out.println("The available options for the optionset "+os.getName()+" are :");
			for(Option opt : os.getOpts()){
				opt.print();
			}
			...
			for(Option opt : os.getOpts()){
				if(selectedOption.equalsIgnoreCase(opt.getName())){
					this.setOptionChoice(os.getName(), opt.getName());
//					System.out.println("Option for "+ os.getName() +"is selected as "+ getOptionChoice(os.getName()) +"\n" );
					validOption = true;
					break;
				}
			}
			if(!validOption)
				System.out.println("The option you selected does not exist!");
		}
	}
	
	/**
	 * Used to print the selected options for an automobile
	 * prints null if no option is selected
	 */
